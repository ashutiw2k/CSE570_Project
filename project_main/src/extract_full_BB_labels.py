...
def extract_bounding_boxes(input_dir, output_file, dataset, visible_threshold=0.90):
    ...
    for idx, fname in enumerate(sorted(os.listdir(input_dir))):
        if fname.lower().endswith(('.png', '.jpg', '.jpeg')) and '.'.join(fname.split('.')[0:2]) in bounding_boxes.keys():
            # print(fname.split('.'))
            # break
            ...
            # print(fname.split('_'))
            timestamp = '.'.join(fname.split('.')[0:2])
...
if __name__ == "__main__":
    ...
    input_dir = "project_main/data/Annotated Images From Pickle/"  # Directory containing masked images
    output_dir = "project_main/data/Full BB Labels/"
