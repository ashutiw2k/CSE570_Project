...
from newutils import split_frame_into_cameras, adjust_bbox_for_camera, compute_iou

def preprocess_virtual_cameras(batch, n):
    """Split batch of video frames into two virtual camera views"""
...
def adjust_bboxes(batch_bboxes, cam1_origin, n):
    """Adjusts bounding boxes - Cam 1"""
    adjusted_bboxes = []
    for bbox in batch_bboxes:
        adjusted_bbox = adjust_bbox_for_camera(bbox, cam1_origin, n)
        if adjusted_bbox:
            adjusted_bboxes.append(adjusted_bbox)
    return adjusted_bboxes

def evaluate_iou(pred_bboxes, gt_bboxes):
    """IoU score between predicted and ground truth BBs"""
    iou_scores = []
