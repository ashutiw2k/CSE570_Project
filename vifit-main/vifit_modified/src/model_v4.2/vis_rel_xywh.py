...
from newutils import adjust_bbox_for_camera, compute_iou


def process_virtual_cameras(train_seq_in_correspondence, cam1_origin, n):
    """Process and adjust BBs for virtual cams"""
    cam1_data, cam2_data = [], []
    for bbox in train_seq_in_correspondence[0]:
        if bbox[0] < cam1_origin[0] + n and bbox[1] > cam1_origin[1] - n:
            cam1_data.append(bbox)  # Camera 1 bounding boxes
        else:
            cam2_data.append(bbox)  # Camera 2 bounding boxes
    return cam1_data, cam2_data

def evaluate_iou_visualization(pred_bboxes, gt_bboxes):
    """Compute and visualize IoU"""
    iou_scores = [compute_iou(pred, gt) for pred, gt in zip(pred_bboxes, gt_bboxes)]
...
def adjust_bboxes_for_visualization(train_seq_in_correspondence, cam1_origin, n):
    cam1_bboxes = [adjust_bbox_for_camera(bbox, cam1_origin, n) for bbox in train_seq_in_correspondence[0]]
    return cam1_bboxes

def visualize_predicted_vs_actual(cam1_pred, cam1_actual, cam2_pred, cam2_actual):
    """Visualize pred and actual BBs for both cams"""
    plt.scatter([b[0] for b in cam1_actual], [b[1] for b in cam1_actual], color='green', label='Camera 1 Actual')
    plt.scatter([b[0] for b in cam1_pred], [b[1] for b in cam1_pred], color='blue', label='Camera 1 Predicted')
    plt.scatter([b[0] for b in cam2_actual], [b[1] for b in cam2_actual], color='orange', label='Camera 2 Actual')
    plt.scatter([b[0] for b in cam2_pred], [b[1] for b in cam2_pred], color='red', label='Camera 2 Predicted')
    plt.legend()
    plt.xlabel('X Coordinate')
    plt.ylabel('Y Coordinate')
    plt.title('Predicted vs Actual Bounding Boxes')
    plt.show()



def proc_rel_xywh(C: Config):
    cam1_data, cam2_data = process_virtual_cameras(train_seq_in_correspondence, cam1_origin=(0, 0), n=200)
    evaluate_iou_visualization(cam1_data, cam2_data)
